...
当任意 Agent 检测到规范变动并暂停任务后，由用户通知 `@TL` 执行规范更新。
...
在更新前，主动确认：
- 变动是否与财务核心律法冲突（如有冲突，必须明确指出，不得直接更新）
- 变动是否影响已完成 Step 的代码（如有，评估是否需要返工）
- 变动是否需要同步调整多个规范文件
...
| 变动类型 | 更新文件 |
|---------|---------|
| 架构 / 模块结构变动 | `docs/ai-rules/general.md` |
| Java 编码规范变动 | `docs/ai-rules/java.md` |
| 账务领域规范变动 | `docs/ai-rules/accounting.md` |
| 某个 Agent 行为变动 | `docs/ai-rules/agents/对应Agent.md` |
| 影响所有工具 | `docs/ai-rules/all-in-one.md`（同步合并更新） |

**3. 输出变更记录**

每次规范更新后，必须输出以下格式的变更记录：

```
## 规范变更记录

- 变更时间：[当前日期]
- 变更文件：[docs/ai-rules/xxx.md]
- 变更章节：[具体章节名]
- 变更摘要：[一句话说明改了什么]
- 变更前：[原内容]
- 变更后：[新内容]
- 影响评估：[是否需要返工，涉及哪些 Step]
- 通知成员：[需要知晓此变更的团队成员]
```
...
- 不得在未经用户确认的情况下自行修改规范文件
- 不得因"影响不大"而跳过变更记录输出
- 财务核心律法相关变动必须重点标注，不得静默更新